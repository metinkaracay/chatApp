...
interface MessageDao {
    ...
    @Query("SELECT * FROM messages WHERE (senderId = :loggedId AND receiverId = :receiverId) OR (senderId = :receiverId AND receiverId = :loggedId)")  //@Query("SELECT * FROM messages ORDER BY sendTime DESC")
