...
interface MessageDao {
    ...
    @Insert
    suspend fun insertAllMessages(vararg messages: MessageItem)

    @Insert
    suspend fun insertMessage(messages: MessageItem)

    @Query("SELECT * FROM messages")  //@Query("SELECT * FROM messages ORDER BY sendTime DESC")
    suspend fun getAllMessages(): List<MessageItem>
